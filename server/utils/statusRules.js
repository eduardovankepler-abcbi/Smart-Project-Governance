...
function normalizeTaskStatus(value, progress = 0) {
  ...
  if (raw.includes("freeze") || raw.includes("freezing") || raw.includes("frozen") || raw.includes("congel")) return TASK_STATUSES.FROZEN;
...
function deriveTaskStatus(task = {}, options = {}) {
  ...
  if (explicitStatus === TASK_STATUSES.FROZEN) return TASK_STATUSES.FROZEN;
...
function deriveProjectStatus(tasks = []) {
  ...
  if (statuses.some((status) => status === TASK_STATUSES.FROZEN)) return TASK_STATUSES.FROZEN;
